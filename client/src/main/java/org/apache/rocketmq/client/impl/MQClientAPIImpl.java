...
public class MQClientAPIImpl {
    ...
    public SendResult sendMessage(
        ...
        //1.RequestCommand,RequestCode是SEND_MESSAGE
        ...
        switch (communicationMode) {
            case ONEWAY:
                //单向发送：用nettyRemotingClient的invokeOneway，无repsonse
                this.remotingClient.invokeOneway(addr, request, timeoutMillis);
                return null;
            case ASYNC:
                //异步发送：用nettyRemotingClient的invokeAsnyc，不用等待repsonse，在callback处理
                ...
                return null;
            case SYNC:
                //同步发送：用nettyRemotingClient的invokeSnyc，等待repsonse.
        ...
        this.remotingClient.invokeAsync(addr, request, timeoutMillis, new InvokeCallback() {
            ...
            public void operationComplete(ResponseFuture responseFuture) {
                ...
                if (null == sendCallback && response != null) {
                    //sendCallback为空，解析sendResult放在context中
                    try {
                ...
                //response不为空，sendCallback不为空，调用sendCallback.onSuccess(sendResult)
                if (response != null) {
                    try {
                        SendResult sendResult = MQClientAPIImpl.this.processSendResponse(brokerName, msg, response);
                    ...
                    //response为空onExceptionImpl处理异常，处理异常信息，重试：递归调用直到不符合条件或者成功
                    producer.updateFaultItem(brokerName, System.currentTimeMillis() - responseFuture.getBeginTimestamp(), true);
